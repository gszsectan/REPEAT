...
def construct_exemplars_ours(model, args, train_exemplar, eval_exemplar, tokenizer, device, mode):
    ...
    if mode == 'random':
        ...
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=1,num_workers=4,pin_memory=True)
        model.eval()
        score = []
        for batch in train_dataloader:
            ...
            with torch.no_grad():
                ...
                score.append(lm_loss.cpu().item())
        train_topk = heapq.nlargest(new_train_replay_size, range(len(score)), score.__getitem__)

        new_train_exemplars = []
        for idx in train_topk:
            new_train_exemplars.append({'func_before':train_exemplar[idx].origin_source, 'vul':train_exemplar[idx].origin_target, \
                                        'task_id':str(task_id), 'score':np.float(score[idx])})
        for idx in old_replay_train_data:
            train_exemplars.extend(old_replay_train_data[idx][:old_train_replay_size])
        train_exemplars.extend(new_train_exemplars)
        ...
        for batch in eval_dataloader:
            ...
            with torch.no_grad():
                lm_loss,logit = model(inputs,label)
                score.append(lm_loss.cpu().item())
        eval_topk = heapq.nlargest(new_eval_replay_size, range(len(score)), score.__getitem__)
        ...
        for clas in train_exemplar_class:
            ...
            train_topk = []
            for i in range(cluster_number):
                class_topk = heapq.nlargest(max(5*cluster_replay_number[i],len(class_score[i])-1), range(len(class_score[i])), class_score[i].__getitem__)
